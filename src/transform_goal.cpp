...
ros::Publisher cmd_vel_body_pub;
...
void goalCallback(const geometry_msgs::PoseStamped::ConstPtr& goal_msg)
{
    ...
    try {
        listener_ptr->waitForTransform("body", "world", 
                                      goal_msg->header.stamp, ros::Duration(1.0));
        listener_ptr->transformPose("body", *goal_msg, goal_body);

        ROS_INFO("=== world tf axes ===");
        ...
        ROS_INFO("=== body tf axes ===");
        ROS_INFO("Position: (%.4f, %.4f, %.4f)", 
                 goal_body.pose.position.x, goal_body.pose.position.y, goal_body.pose.position.z);
        ROS_INFO("Quaternion: (x=%.4f, y=%.4f, z=%.4f, w=%.4f)",
                 goal_body.pose.orientation.x, goal_body.pose.orientation.y,
                 goal_body.pose.orientation.z, goal_body.pose.orientation.w);
...
void cmdCallback(const quadrotor_msgs::PositionCommand::ConstPtr& cmd_msg)
{
    ...
    // Transform linear velocity and angular (yaw_dot) into 'body' frame
    geometry_msgs::Vector3Stamped lin_world, lin_body;
    geometry_msgs::Vector3Stamped ang_world, ang_body;
    ...
    try {
        listener_ptr->transformVector("body", lin_world, lin_body);
        listener_ptr->transformVector("body", ang_world, ang_body);
        transformed = true;
    }
    catch (tf::TransformException &ex) {
    ...
    if (transformed && go_val) {
        ...
        twist_msg.linear = lin_body.vector;
        twist_msg.angular = ang_body.vector;
        // enforce zero for z linear and x/y angular components
        twist_msg.linear.z = 0.0;
        twist_msg.angular.x = 0.0;
        twist_msg.angular.y = 0.0;

        cmd_vel_body_pub.publish(twist_msg);
    }
}
...
int main(int argc, char** argv)
{
    ...
    cmd_vel_body_pub = nh.advertise<geometry_msgs::Twist>("/cmd_vel_quadruped", 10);
